...
async def on_ready():
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name="you suffer"))
...
async def ping(ctx):
    e = [
        "**Pang!** <:pangblush:649970659358933002>",
...
async def pull_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        if (sec < 60):
            msg = '<:puggiFIRE:808672592550297631> PUGGI IS ON FIRE! Cooldown in **{:.0f}** seconds!'.format(
                sec)
        else:
            minut = sec // 60
            sec2 = sec % 60
            msg = '<:puggiFIRE:808672592550297631> PUGGI IS ON FIRE! Cooldown in **{:.0f}** minutes and **{:.0f}** seconds!'.format(
                minut, sec2)
